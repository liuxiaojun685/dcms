...
function getPrivateList(index, filter) {
	...
	if(filter==1){
		...
		fileLevel = $("#queryByLevelIncreate option:selected").val();
		fileState = $("#queryByStateIncreate option:selected").val();
		keyword = $("#keywordIncreate").val();
	}
    $.ajax({
        type: "post",
        ...
        success: function (result) {
            if (result.code == 0) {
                if (filter == 1) {
                    ...
                    $(".createfileList").empty();
                    $('#createfilepage').empty();
                } else if (filter == 3) {
                    $(".decryptList").empty();
                } else {
                ...
                if (filter == 1) {
                	$(".inboxList").append(str);
                } else if (filter == 2) {
                	$(".createfileList").append(str);
                } else if (filter == 3) {
                ...
                if (filter == 1) {
                    $('#inboxpage').bootstrapPaginator(options);
                    $('#inboxpageTotal').text('共'+result.totalElements+'条');
                } else if (filter == 2) {
                    $('#createfilepage').bootstrapPaginator(options);
                    $('#createfilepageTotal').text('共'+result.totalElements+'条');
                } else if (filter == 3) {
...
//追踪
function track(fid, filter, name){
	...
	if (filter == 1) {
     ...
     // 基于准备好的dom，初始化echarts实例
        myChart = echarts.init(document.getElementById('inboxTrack'));
    } else if (filter == 2) {
        $(".createfileAll").hide();
        $(".createfileInfoAll").empty();
        $("#createfileTrack").empty();
        $(".createfiletrack").show();
        myChart = echarts.init(document.getElementById('createfileTrack'));
    } else if (filter == 3) {
        $(".decryptAll").hide();
    ...
    $.ajax({
        ...
        success: function (result) {
        	if(result.code==0){
        		...
        		if(filter==1){
        			$(".inboxback").unbind("click").click(function(){
            			gotoback(fid, filter);
            		});
        		}else if(filter==2){
        			$(".createfileback").unbind("click").click(function(){
            			gotoback(fid, filter);
            		});
        		}
        	}
...
function findFileById(fid, filter, type) {
	...
	if(type==1){
		...
		if (filter == 1) {
			$(".inboxAll").hide();
			$(".inboxHistoryList").empty();
			$(".inboxHistory").show();
			$(".inboxHistory nav").remove();
		} else if (filter == 2) {
			$(".createfileAll").hide();
			$(".createfileHistoryList").empty();
			$(".createfileHistory").show();
			$(".createfileHistory nav").remove();
		} else if (filter == 3) {
			$(".decryptAll").hide();
...
function getHistory(result, filter){
	...
	if(filter==1){
		...
		$(".inboxHistoryList").append(strinfo);
		$(".inboxHistory").append(str);
	} else if (filter == 2) {
		$(".createfileHistoryList").append(strinfo);
		$(".createfileHistory").append(str);
	} else if (filter == 3) {
...
function getInfoTable(result, filter) {
    ...
    var strinfo = '<div style="width:800px;margin:0 auto;background: #fff;"><table style="margin:0 auto;border: 1px solid #4777c8;">' +
        '<tr>' +
        '<td style="border:1px solid #4777c8"><h4 style="font-weight: bold;text-align: center;">文件详情</h4></td>' +
        '</tr>' +
        '<tr>' +
        '<td class="rowname" style="border-right:1px solid #4777c8"><span style="letter-spacing: 4px;">文件名称：</span><input style="width:390px;margin-left:3px;"  readonly value="' + result.fileName + '"/><span style="padding-left:5px;">标志版本：</span><input readonly  value="' + result.markVersion + '"/></td>' +
        '</tr>' +
        '<tr>' +
        '<td class="rowname" style="border-right:1px solid #4777c8"><span style="letter-spacing: 0;">文件起草人：</span><input style="margin-left:4px;width:164px;" readonly value="' + isUndefined(result.fileCreateUserName) + '"/><span style="letter-spacing: 0;padding-left:4px;">定密责任人：</span><input style="margin-left:2px;width:164px;" readonly value="' + isUndefined(result.fileLevelDecideRoleName) + '"/><span style="letter-spacing: 0;padding-left:5px;">文件签发人：</span><input style="margin-left:2px;width:164px;" readonly value="' + isUndefined(result.fileDispatchRoleName) + '"/></td>' +
        '</tr>' +
        '<tr>' +
        '<td class="rowname" style="border-right:1px solid #4777c8"><span style="letter-spacing: 4px;">起草日期：</span><input style="margin-left:3px;width:164px;" readonly value="' + getDate(result.fileCreateTime) + '"/> <span>定密日期：</span><input style="margin-left:3px;width:164px;" readonly value="' + getDate(result.fileLevelDecideTime) + '"/> <span>签发日期：</span><input style="margin-left:3px;width:164px;" readonly value="' + getDate(result.fileDispatchTime) + '"/></td>' +
        '</tr>' +
        '<tr>' +
        '<td class="rowname" style="border-right:1px solid #4777c8"><span style="letter-spacing: 4px;">秘密等级：</span><input style="width:164px;margin-left:4px;" type="text" readonly value="' + result.fileLevel + '"><span style="padding-left:5px;letter-spacing:3px;">紧急程度：</span><input style="margin-left:3px;width:164px;" readonly  value="' + setfileurgency(result.urgency) + '"/>' +
        '</td>' +
        '</tr>' +
		'<tr>' +
		'<td colspan="2">'+
		'<span class="rowname" style="padding-left:2px;">保密期限：</span>' +
		'<select id="pdurationType" name="durationType" disabled class="time1" style="width:166px;margin-left:6px;margin-top:3px;">' +
        ...
        '</select>' +
        '<div class="ptimes ptime1" style="display: inline-block;margin-bottom:-35px;"></div>' +
		'</td>' +
		'</tr>' +
        '<tr>' +
        '<td class="rowname" style="border-right:1px solid #4777c8"><span style="letter-spacing: 0;">主定密单位：</span><input style="width:690px;margin-left:3px;" type="text"readonly value="' + result.fileMajorUnit.name + '"/></td>' +
        '</tr>' +
        '<tr>' +
        '<td class="rowname" style="border-right:1px solid #4777c8"><span style="letter-spacing: 0;">辅定密单位：</span>' +
        '<div class="list-major-input" style="border-left: 0;height: 37px;display:inline-block;width: 80%;margin-left:95px;margin-top:-40px;">' +
        '<div class="i_inp" style="overflow-y: auto;width: 690px;height: 40px;top:10px;">' +
        '<span class="ui-sortable pdisp " style="font-size: 12px;"></span>' +
        '</div>' +
        '</div>' +
        '</td>' +
        '</tr>' +
        ...
        '<select id="pbasisType" name="basisType" disabled style="margin-top:15px;margin-left:7px;">' +
        ...
        '<textarea id="pid_basisDescription" style="width: 680px;height: 35px;overflow-y: auto;color:#000;margin-left:93px;"></textarea>' +
        '</td>' +
        '</tr>' +
        ' <tr>' +
        '<td class="rowname" style="border-right:1px solid #4777c8"><span>知悉范围：</span>' +
        '<div class="box" style="border: 0;display: inline-block;margin-left:95px;margin-top:-40px;">' +
        ...
        '<textarea id="ptooltips" rows="2" readonly class="chooseList" style="resize:none;width:680px;"></textarea>' +
        '<textarea id="pscopeDesc" rows="2" readonly style="display: none;resize:none;width:680px;">' + result.fileScopeDesc + '</textarea>' +
        ' </div>' +
        ' </div>' +
        '</div>' +
        '</div>' +
        ...
        '<div class="ppermission" style="display:inline-block;margin-left:95px;margin-top:-30px;">' +
        '</div>' +
        '</td>' +
        '</tr>' +
        ' <tr>' +
        ...
        '<span>文号：</span>' +
        '<input value="' + result.issueNumber + '" style="width:170px;" readonly name="issueNumber" type="text" />' +
        '<span> 份号：</span>' +
        '<input readonly name="docNumber" type="text" style="width:170px;" value="' + result.docNumber + '"/>' +
        '<span>份数：</span><input readonly name="duplicationAmount" style="width:170px;" type="text" value="' + result.duplicationAmount + '"/>' +
        '</td>' +
        '</tr>' +        
        '<tr>'+
	    '<td class="rowname" colspan="2" style="border-right:1px solid #4777c8">' +
	    '<span>业务属性：</span>' +
	    '<textarea style="overflow-y: auto;color:#000;width:690px;" rows="3">'+result.business+'</textarea>' +
	    '</td>' +
	    '</tr>' +
        '</table>' +
    ...
    if (filter == 1) {
        $(".inboxInfoAll").append(strinfo);
    } else if (filter == 2) {
        $(".createfileInfoAll").append(strinfo);
    } else if (filter == 3) {
...
function gotoback(fid, filter) {
    if (filter == 1) {
        ...
        $(".createfileAll").show();
        $(".createfileInfoAll").hide();
        $(".createfileInfoHistory").hide();
        $(".createfiletrack").hide();
    } else if (filter == 3) {
        $(".decryptAll").show();
        $(".decryptInfoAll").hide();
        $(".decryptHistory").hide();
    ...
    if (units.length > 0) {
        for (var j = 0; j < units.length; j++) {
            ...
            if (parten.test(rds)) {
                ...
                $(".pdisp").append('<span id="dc" class="sort"  style="white-space:nowrap; float:left; display:block;   margin:5px;   height:22px; font-size:12px; "><div  class="dartUnit"  style="padding:0px 6px; cursor:move; height:22px; display:block; float:left;   line-height:22px;">' + rds + '</div>').sortable("refresh");
...
function filePermissionData(dispatch) {
    ...
    if (dispatch.contentPrintHideWater == 1) {
        str += '<input type="checkbox"  name="contentPrintHideWater" checked>打印隐藏水印';
    } else {
        str += '<input type="checkbox"  name="contentPrintHideWater" >打印隐藏水印';
    }
