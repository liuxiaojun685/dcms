...
function getInfoTable(result, filter) {
    ...
    var strinfo = '<div style="background: #fff;width:800px;margin:0 auto;"><table style="margin:0 auto;border: 1px solid #4777c8;">' +
        '<tr>' +
        '<td style="border:1px solid #4777c8"><h4 style="font-weight: bold;text-align: center;">文件详情</h4></td>' +
        '</tr>' +
        '<tr>' +
        '<td class="rowname" style="border-right: 1px solid #4777c8;"><span style="letter-spacing:4px;">文件名称：</span><input style="width:390px;margin-left:1px;"  readonly value="' + result.fileName + '"/><span style="padding-left:5px;">标志版本：</span><input readonly  value="' + result.markVersion + '"/></td>' +
        '</tr>' +
        '<tr>' +
        '<td class="rowname" style="border-right: 1px solid #4777c8;"><span style="letter-spacing: 0;">文件起草人：</span><input style="margin-left:3px;width:164px;" readonly value="' + isUndefined(result.fileCreateUserName) + '"/><span style="letter-spacing: 0;padding-left:4px;">定密责任人：</span><input style="margin-left:2px;width:164px;" readonly value="' + isUndefined(result.fileLevelDecideRoleName) + '"/><span style="letter-spacing: 0;padding-left:4px;">文件签发人：</span><input style="margin-left:2px;width:164px;" readonly value="' + isUndefined(result.fileDispatchRoleName) + '"/></td>' +
        '</tr>' +
        '<tr>' +
        '<td class="rowname" style="border-right: 1px solid #4777c8;"><span style="letter-spacing:4px;>起草日期：</span><input style="margin-left:3px;width:164px;" readonly value="' + getDate(result.fileCreateTime) + '"/> <span>定密日期：</span><input style="margin-left:3px;width:164px;" readonly value="' + getDate(result.fileLevelDecideTime) + '"/> <span style="letter-spacing:3px;">签发日期：</span><input style="margin-left:3px;width:164px;" readonly value="' + getDate(result.fileDispatchTime) + '"/></td>' +
        '</tr>' +
        '<tr>' +
		...
		'<tr>' +
		'<td>'+
		'<span style="letter-spacing:3px;" class="rowname">保密期限：</span>' +
		'<select id="pdurationType" name="durationType" disabled class="time1" style="margin-top:3px;margin-left:8px;width:166px;">' +
        ...
        '<div id="pcontent" class="list-name-select" style="width: 560px;border-left: 0;height: 37px;display:inline-block;margin-left:93px;">' +
        '<div class="i_inpBasis" style="width:680px;overflow-y: auto;height: 40px;">' +
        '<textarea id="pid_basisDescription" style="width: 660px;height: 35px;overflow-y: auto;color:#000;"></textarea>' +
        '</span>' +
        '</div>' +
        '</div>' +
        '</td>' +
        ...
        '</tr>' +
        '<tr>' +
        '<td colspan="2"  class="rowname" style="border-right: 1px solid #4777c8;"><span>安全控制：</span>' +
        '<div class="ppermission" style="display:inline-block;margin-left:90px;margin-top:-30px;">' +
        '</div>' +
        '</td>' +
        '</tr>' +
        ' <tr>' +
        ...
        '<span>文号：</span>' +
        '<input style="width:170px;" value="' + result.issueNumber + '" readonly name="issueNumber" type="text" />' +
        '<span> 份号：</span>' +
        '<input style="width:170px;" readonly name="docNumber" type="text" value="' + result.docNumber + '"/>' +
        '<span>份数：</span>'+
		'<input style="width:170px;" readonly name="duplicationAmount" type="text" value="' + result.duplicationAmount + '"/>' +
        '</td>' +
        '</tr>' +       
        '<tr>'+
	    '<td class="rowname" colspan="2" style="border-right: 1px solid #4777c8;">' +
	    '<span>业务属性：</span>' +	            
	    '<textarea style="overflow-y: auto;color:#000;width:685px;" rows="3">'+result.business+'</textarea>' +
	    '</td>' +
	    '</tr>' +
        '</table>' +
...
function filePermissionData(dispatch) {
    ...
    if (dispatch.contentRead == 1) {
        str += '<input type="checkbox" style="border:0;"  name="contentRead" checked>内容阅读';
    } else {
        str += '<input type="checkbox" style="border:0;"   name="contentRead" >内容阅读';
    }
    if (dispatch.contentModify == 1) {
        str += '<input type="checkbox" style="border:0;"   name="contentModify" checked>内容修改';
    } else {
        str += '<input type="checkbox" style="border:0;"   name="contentModify" >内容修改';
    }
    if (dispatch.contentPrint == 1) {
        str += '<input value="1" type="checkbox" style="border:0;"   name="contentPrint" checked>打印';
    } else {
        str += '<input value="1" type="checkbox" style="border:0;"   name="contentPrint" >打印';
    }
    if (dispatch.contentPrintHideWater == 1) {
        str += '<input type="checkbox" style="border:0;"   name="contentPrintHideWater" checked>打印隐藏水印';
    } else {
        str += '<input type="checkbox" style="border:0;"   name="contentPrintHideWater" >打印隐藏水印';
    }

    if (dispatch.contentScreenShot == 1) {
        str += '<input type="checkbox" style="border:0;"   name="contentScreenShot" checked>截屏';
    } else {
        str += '<input type="checkbox" style="border:0;"   name="contentScreenShot" >截屏';
    }
    if (dispatch.contentCopy == 1) {
        str += '<input type="checkbox" style="border:0;"   name="contentCopy" checked>内容拷贝';
    } else {
        str += '<input type="checkbox" style="border:0;"   name="contentCopy" >内容拷贝';
    }
    if (dispatch.fileCopy == 1) {
        str += '<input type="checkbox" style="border:0;"   name="fileCopy" checked>文件拷贝';
    } else {
        str += '<input type="checkbox" style="border:0;"   name="fileCopy" >文件拷贝';
    }

    if (dispatch.fileSaveCopy == 1) {
        str += '<input type="checkbox" style="border:0;"   name="fileSaveCopy" checked>另存副本';
    } else {
        str += '<input type="checkbox" style="border:0;"   name="fileSaveCopy" >另存副本';
    }
    $(".ppermission").empty();
