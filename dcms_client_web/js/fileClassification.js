...
function loadScopeTree(s) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.code == 0) {
                var str = '<ul id="id_scopeTree" class="filetree treeview"><li onclick="onAccessScopeTreeNodeClick(this.id)"><span class="com" style="color: #000;"><input type="checkbox" onclick="onAccessScopeTreeNodeClick(this.id)" value=' + data.did + ' id="' + data.did + '">' + data.name + '</span>' + installScopeTree(data) + '</li></ul>';
                $("#id_scopeTree").empty();
        ...
        success: function (data) {
            if (data.code == 0) {
                ...
                if (uploadType == 9) {
                    ...
                    var obj = new Array();
                    obj.push(header);
                    ocx.saveFileHeader(JSON.stringify(obj), needUpload, JSON.stringify(uploadParam));
                }
            } else {
