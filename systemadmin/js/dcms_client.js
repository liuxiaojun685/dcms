            ...
            result.onSuccess = function () {
                ...
                for (var i = 0; i < result.clientList.length; i++) {
                    ...
                    var str = '<tr>' +
                        ...
                        '<td class="text-center">' + isUndefined(result.clientList[i].pcName) + '</td>' +
                        '<td class="text-center">' + isUndefined(result.clientList[i].ip) + '</td>' +
                        '<td class="text-center">' + isUndefined(result.clientList[i].mac) + '</td>' +
                        '<td class="text-center">' + level + '</td>' +
                        '<td class="text-center">' + versionType + '</td>' +
                        '<td class="text-center">' + isUndefined(result.clientList[i].versionCode) + '</td>' +
                        '<td class="text-center">' + online + '</td>' +
                        '<td class="description text-center">' + isUndefined(result.clientList[i].description) + '</td>' +
                        '<td class="text-center"><a href="javaScript:;;" onclick="editsave(this)"><i class="fa fa-edit"></i></a><span style="padding:5px 10px;"></span><a' +
                        ' href="javaScript:;;"' +
...
function editsave(t) {
    ...
    if (str.hasClass("fa-edit")) {
        ...
        if (!obj_text.length) {   // 如果没有文本框，则添加文本框使之可以编辑
            ...
            //设置文本框宽度等于td的宽度
            $(".editinput").width(tdenbale.width()+100);
			alert(tdenbale.width())
            $(".editinput").focus();
            //当文本框失去焦点时重新变为文本
