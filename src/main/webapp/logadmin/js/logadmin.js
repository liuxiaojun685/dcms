        ...
        success: function (result) {
            result.onSuccess = function () {
                ...
                for (var i = 0; i < datas.length; i++) {
                    ...
                    if (datas[i].admin.account != null) {
                        str += '<td>' + datas[i].admin.account + '</td>';
                    }
                    str += '<td>' + operateTime + '</td>' +
                        '<td>' + datas[i].operateType + '</td>' +
                        '<td>' + datas[i].operateDescription + '</td>' +
                        '<td>' + datas[i].ip + '</td>';
                    if (datas[i].riskLevel == "一般") {
                        str += '<td><i class="text" style="padding:0 10px;background:#666666; "></i><span style="margin-left:5px;">' + datas[i].riskLevel + '</span></td>';
                    }
                    if (datas[i].riskLevel == "较重") {
                        str += '<td><i class="text" style="padding:0 10px;background:#cc3300; "></i><span style="margin-left:5px;">' + datas[i].riskLevel + '</span></td>';
                    }
                    if (datas[i].riskLevel == "严重") {
                        str += '<td><i class="text" style="padding:0 10px;background:#ff0000; "></i><span style="margin-left:5px;">' + datas[i].riskLevel + '</span></td>';
                    }
                    str += '</tr>';
                    $(".list").append(str);
